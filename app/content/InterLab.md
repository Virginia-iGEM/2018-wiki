# InterLab   


We did this year's iGEM InterLab study!     
Our original Excel file can be downloaded here.      
Here is the protocol that we followed.     

## OD600 Reference Point - LUDOX Protocol

<table style="width:100%" id="parts">
	<tr>
		<th></th>
		<th>LUDOX CL-X</th> 
		<th>H2O</th>
	</tr>
	<tr>
		<td>Replicate 1</td>
		<td>0.113</td>
		<td>0.087</td>
	</tr>
	<tr>
		<td>Replicate 2</td>
		<td>0.105 </td>
		<td>0.085</td>
	</tr>
	<tr>
		<td>Replicate 3</td>
		<td>0.119 </td>
		<td>0.084</td>
	</tr>
	<tr>
		<td>Replicate 4</td>
		<td>0.103 </td>
		<td>0.085</td>
	</tr>
	<tr>
		<td>Arith. Mean</td>
		<td>0.110</td>
		<td>0.085</td>
	</tr>
	<tr>
		<td>Corrected Abs600</td>
		<td>0.025 </td>
		<td></td>
	</tr>
	<tr>
		<td>Reference OD600</td>
		<td>0.063</td>
		<td> </td>
	</tr>
	<tr>
		<td>OD600/Abs600</td>
		<td>2.545 </td>
		<td></td>
	</tr>
</table>

## Particle Standard Curve: Microsphere Protocol

Need to color code but:
Abs600 measurements are blue cells.
Gold cells are calculated. 


<table style="width:100%" id="parts">
	<tr>
		<th>Number of Particles</th>
		<th>2.35E+08</th> 
		<th>1.18E+08</th>
		<th>5.88E+07</th>
		<th>2.94E+07</th>
		<th>1.47E+07</th>
		<th>7.35E+06</th>
		<th>3.68E+06</th>
		<th>1.84E+06</th>
		<th>9.19E+05</th>
		<th>4.60E+05</th>
		<th>2.30E+05</th>
		<th>0</th>
	</tr>
	<tr>
		<td>Replicate 1</td>
		<td>0.534</td>
		<td>0.337</td>
		<td>0.211</td>
		<td>0.148</td>
		<td>0.124</td>
		<td>0.102</td>
		<td>0.094</td>
		<td>0.091</td>
		<td>0.089</td>
		<td>0.089</td>
		<td>0.088</td>
		<td>0.087</td>
	</tr>
	<tr>
		<td>Replicate 2</td>
		<td>0.538</td>
		<td>0.329</td>
		<td>0.204</td>
		<td>0.145</td>
		<td>0.117</td>
		<td>0.102</td>
		<td>0.099</td>
		<td>0.092</td>
		<td>0.089</td>
		<td>0.089</td>
		<td>0.086</td>
		<td>0.085</td>
	</tr>
	<tr>
		<td>Replicate 3</td>
		<td>0.543</td>
		<td>0.332</td>
		<td>0.209</td>
		<td>0.145</td>
		<td>0.116</td>
		<td>0.107</td>
		<td>0.093</td>
		<td>0.090</td>
		<td>0.088</td>
		<td>0.089</td>
		<td>0.086</td>
		<td>0.083</td>
	</tr>
	<tr>
		<td>Replicate 4</td>	
		<td>0.545</td>
		<td>0.314</td>
		<td>0.208</td>
		<td>0.143</td>
		<td>0.119</td>
		<td>0.102</td>
		<td>0.092</td>
		<td>0.089</td>
		<td>0.089</td>
		<td>0.089</td>
		<td>0.086</td>
		<td>0.083</td>
	</tr>
	<tr>
		<td>Arith. Mean</td>	
		<td>0.540</td>
		<td>0.328</td>
		<td>0.208</td>
		<td>0.145</td>
		<td>0.119</td>
		<td>0.103</td>
		<td>0.095</td>
		<td>0.091</td>
		<td>0.089</td>
		<td>0.089</td>
		<td>0.087</td>
		<td>0.085</td>
	</tr>
	<tr>
		<td>Arith. Std.Dev.	</td>
		<td>0.005</td>
		<td>0.010</td>
		<td>0.003</td>
		<td>0.002</td>
		<td>0.004</td>
		<td>0.003</td>
		<td>0.003</td>
		<td>0.001</td>
		<td>0.001</td>
		<td>0.000</td>
		<td>0.001</td>
		<td>0.002</td>
	</tr>
	<tr>
		<td>Arith. Net Mean</td>	
		<td>0.456</td>
		<td>0.244</td>
		<td>0.124</td>		
		<td>0.061</td>
		<td>0.035</td>
		<td>0.019</td>
		<td>0.010</td>
		<td>0.006</td>
		<td>0.004</td>
		<td>0.004</td>
		<td>0.002</td>	
		<td></td>
	</tr>
</table>

<img src="/images/InterLab/ParticleStandardCurve.png" alt="ParticleStandardCurve">
<img src="/images/InterLab/ParticleStandardCurve_log.png" alt="ParticleStandardCurve_log">

### Particles/OD

Note: Final scaling level determined from medium-high points likely to be less impacted by saturation or pipetting error. If needed, you can shift which points are used, but it is likely better to correct instrument settings and protocol.


<table style="width:100%" id="parts">
	<tr>
		<th>Number of Particles</th>	
		<th>2.35E+08</th>
		<th>1.18E+08</th>
		<th>5.88E+07</th>
		<th>2.94E+07</th>
		<th>1.47E+07</th>
		<th>7.35E+06</th>
		<th>3.68E+06</th>
		<th>1.84E+06</th>
		<th>9.19E+05</th>
		<th>4.60E+05</th>
		<th>2.30E+05</th>
	</tr>
	<tr>
		<td>Mean particles / Abs600	</td>
		<td>5.17E+08</td>
		<td>4.83E+08</td>
		<td>4.76E+08</td>
		<td>4.84E+08</td>
		<td>4.26E+08</td>
		<td>3.92E+08</td>
		<td>3.68E+08</td>
		<td>3.06E+08</td>
		<td>2.16E+08</td>
		<td>1.02E+08</td>
		<td>1.15E+08</td>
	</tr>
	<tr>
		<td>Mean of med-high levels:</td>
		<td></td>
		<td>4.52E+08</td>
		<td></td>
		<td></td>
		<td></td>
		<td></td>
		<td></td>
		<td></td>
		<td></td>
		<td></td>
		<td></td>
	</tr>
</table>

### Cospheric Monodisperse Silica Microspheres 0.961um diameter

<table style="width:100%" id="parts">
	<tr>
		<th>Spheres/gram</th>	
		<th>Grams/mL</th>
		<th>Spheres/0.55mL</th>
		<th>Resuspend volume mL:</th>
		<th>Dilution X:</th>
		<th>Total volume mL:</th>
		<th>Particles/mL:</th>
		<th>Well volume (mL)</th>
		<th>Initial particles:</th>
	</tr>
	<tr>
		<td>1.20E+12</td>
		<td>1.8</td>
		<td>1.19E+12</td>
		<td>2.55</td>
		<td>100</td>
		<td>255</td>
		<td>4.71E+09</td>
		<td>0.1</td>
		<td>4.71E+08</td>
	</tr>
</table>


## Raw Plate Reader Measurements 

### Fluorescence Raw Readings: 

<table style="width:100%" id="parts">
	<tr>
		<th>Hour 0:	</th>
		<th>Neg. Control</th>
		<th>Pos. Control</th>
		<th>Device 1</th>
		<th>Device 2</th>
		<th>Device 3</th>
		<th>Device 4</th>
		<th>Device 5</th>
		<th>Device 6</th>
		<th>LB + Chlor (blank)</th>
	</tr>
	<tr>
		<td>Colony 1, Replicate 1</td>	
		<td>124.47</td>
		<td>172.15</td>
		<td>136.79</td>
		<td>137.65</td>
		<td>115.66</td>
		<td>132.36</td>
		<td>127.35</td>
		<td>135.34</td>
		<td>112.83</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 2</td>
		<td>121.01</td>
		<td>142.23</td>
		<td>113.72</td>
		<td>130.07</td>
		<td>93.041</td>
		<td>106.03</td>
		<td>104.61</td>
		<td>113.2</td>
		<td>98.307</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 3</td>	
		<td>123.38</td>	
		<td>148.37</td>	
		<td>112.73</td>	
		<td>131.37</td>	
		<td>90.708</td>	
		<td>106.35</td>	
		<td>104.49</td>	
		<td>113.95</td>	
		<td>95.141</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 4</td>	
		<td>119.97</td>
		<td>141.57</td>
		<td>113.49</td>
		<td>124.8</td>
		<td>93.35</td>
		<td>107.29</td>
		<td>102.57</td>
		<td>111.37</td>
		<td>96.012</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 1</td>
		<td>121.32</td>
		<td>134.7</td>
		<td>116.21</td>
		<td>169.04</td>
		<td>93.865</td>
		<td>110.22</td>
		<td>152.05</td>
		<td>111.67</td>
		<td>99.513</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 2</td>
		<td>134.51</td>
		<td>141.18</td>
		<td>115.5</td>
		<td>131.01</td>
		<td>94.94</td>
		<td>107.69</td>
		<td>124.91</td>
		<td>114.24</td>
		<td>96.457</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 3</td>
		<td>130.47</td>
		<td>141.41</td>
		<td>114.54</td>
		<td>130.7</td>
		<td>92.262</td>
		<td>111.95</td>
		<td>128.96</td>
		<td>115.03</td>
		<td>98.671</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 4</td>
		<td>129.77</td>
		<td>160.97</td>
		<td>142.19</td>
		<td>130.7</td>
		<td>112.12</td>
		<td>128.64</td>
		<td>155.08</td>
		<td>129.88</td>
		<td>122.04</td>
	</tr>
</table>

<table style="width:100%" id="parts">
	<tr>
		<th>Hour 6:	</th>
		<th>Neg. Control</th>
		<th>Pos. Control</th>
		<th>Device 1</th>
		<th>Device 2</th>
		<th>Device 3</th>
		<th>Device 4</th>
		<th>Device 5</th>
		<th>Device 6</th>
		<th>LB + Chlor (blank)</th>
	</tr>
	<tr>
		<td>Colony 1, Replicate 1</td>	
		<td>147.58</td>	
		<td>923.47</td>
		<td>481.57</td>
		<td>768.79</td>
		<td>145.84</td>
		<td>158.29</td>
		<td>196.74</td>
		<td>472.33</td>
		<td>130.21</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 2</td>
		<td>135.57</td>	
		<td>868.37</td>
		<td>443.8</td>
		<td>771.52</td>
		<td>133.54</td>
		<td>150.48</td>
		<td>174.77</td>
		<td>442.69</td>
		<td>108.87</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 3</td>	
		<td>133.91</td>
		<td>821.45</td>
		<td>435.09</td>
		<td>749.58</td>
		<td>135.07</td>
		<td>147.14</td>
		<td>176.84</td>
		<td>433.8</td>
		<td>106.07</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 4</td>	
		<td>131.16</td>	
		<td>825.34</td>
		<td>424.75</td>
		<td>702.31</td>
		<td>136.56</td>
		<td>145.48</td>
		<td>178.85</td>
		<td>435.43</td>
		<td>101.34</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 1</td>
		<td>129.25</td>	
		<td>736.73</td>
		<td>506.04</td>
		<td>721.65</td>
		<td>130.79</td>
		<td>152.77</td>
		<td>263.05</td>
		<td>404.78</td>
		<td>105.13</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 2</td>
		<td>129.93</td>
		<td>746.95</td>
		<td>522.99</td>
		<td>752.02</td>
		<td>130.12</td>
		<td>154.08</td>
		<td>274.44</td>
		<td>99.78</td>
		<td>102.2</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 3</td>
		<td>134.9</td>	
		<td>737.42</td>
		<td>519.61</td>
		<td>760.98</td>
		<td>129.32</td>
		<td>162.53</td>
		<td>277.69</td>
		<td>390.5</td>
		<td>104.1</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 4</td>
		<td>136.44</td>	
		<td>824.81</td>
		<td>585.7</td>
		<td>749.59</td>
		<td>145.38</td>
		<td>171.03</td>
		<td>319.39</td>
		<td>429.11</td>
		<td>122.55</td>
	</tr>
</table>

### Abs600 Raw Readings

<table style="width:100%" id="parts">
	<tr>
		<th>Hour 0:	</th>
		<th>Neg. Control</th>
		<th>Pos. Control</th>
		<th>Device 1</th>
		<th>Device 2</th>
		<th>Device 3</th>
		<th>Device 4</th>
		<th>Device 5</th>
		<th>Device 6</th>
		<th>LB + Chlor (blank)</th>
	</tr>
	<tr>
		<td>Colony 1, Replicate 1</td>	
		<td>0.114</td>	
		<td>0.112</td>
		<td>0.112</td>
		<td>0.116</td>
		<td>0.115</td>
		<td>0.113</td>
		<td>0.11</td>
		<td>0.112</td>
		<td>0.097</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 2</td>
		<td>0.113</td>	
		<td>0.112</td>
		<td>0.108</td>
		<td>0.108</td>
		<td>0.109</td>
		<td>0.109</td>
		<td>0.106</td>
		<td>0.112</td>
		<td>0.097</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 3</td>	
		<td>0.114</td>	
		<td>0.11</td>
		<td>0.106</td>
		<td>0.108</td>
		<td>0.109</td>
		<td>0.108</td>
		<td>0.106</td>
		<td>0.111</td>
		<td>0.093</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 4</td>	
		<td>0.114</td>	
		<td>0.11</td>
		<td>0.106</td>
		<td>0.108</td>
		<td>0.109</td>
		<td>0.107</td>
		<td>0.105</td>
		<td>0.109</td>
		<td>0.095</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 1</td>
		<td>0.115</td>	
		<td>0.114</td>
		<td>0.108</td>
		<td>0.123</td>
		<td>0.111</td>
		<td>0.112</td>
		<td>0.124</td>
		<td>0.114</td>
		<td>0.094</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 2</td>
		<td>0.115</td>	
		<td>0.112</td>
		<td>0.107</td>
		<td>0.108</td>
		<td>0.109</td>
		<td>0.109</td>
		<td>0.11</td>
		<td>0.111</td>
		<td>0.096</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 3</td>
		<td>0.114</td>	
		<td>0.111</td>
		<td>0.107</td>
		<td>0.107</td>
		<td>0.11</td>
		<td>0.111</td>
		<td>0.116</td>
		<td>0.111</td>
		<td>0.093</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 4</td>
		<td>0.115</td>	
		<td>0.113</td>	
		<td>0.109</td>	
		<td>0.11</td>
		<td>0.11</td>
		<td>0.109</td>	
		<td>0.11</td>
		<td>0.11</td>
		<td>0.095</td>
	</tr>
</table>

<table style="width:100%" id="parts">
	<tr>
		<th>Hour 6:	</th>
		<th>Neg. Control</th>
		<th>Pos. Control</th>
		<th>Device 1</th>
		<th>Device 2</th>
		<th>Device 3</th>
		<th>Device 4</th>
		<th>Device 5</th>
		<th>Device 6</th>
		<th>LB + Chlor (blank)</th>
	</tr>
	<tr>
		<td>Colony 1, Replicate 1</td>	
		<td>0.585</td>	
		<td>0.526</td>
		<td>0.49</td>
		<td>0.538</td>
		<td>0.594</td>
		<td>0.545</td>
		<td>0.558</td>
		<td>0.595</td>
		<td>0.096</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 2</td>
		<td>0.563</td>	
		<td>0.533</td>
		<td>0.483</td>
		<td>0.533</td>
		<td>0.582</td>
		<td>0.544</td>
		<td>0.541</td>
		<td>0.56</td>
		<td>0.093</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 3</td>
		<td>0.595</td>	
		<td>0.538</td>
		<td>0.498</td>
		<td>0.525</td>
		<td>0.585</td>
		<td>0.566</td>
		<td>0.54</td>
		<td>0.531</td>
		<td>0.094</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 4</td>	
		<td>0.586</td>
		<td>0.532</td>
		<td>0.49</td>
		<td>0.494</td>
		<td>0.581</td>
		<td>0.571</td>
		<td>0.5</td>
		<td>0.569</td>
		<td>0.094</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 1</td>
		<td>0.548</td>	
		<td>0.543</td>
		<td>0.502</td>
		<td>0.527</td>
		<td>0.553</td>
		<td>0.565</td>
		<td>0.544</td>
		<td>0.564</td>
		<td>0.094</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 2</td>
		<td>0.566</td>	
		<td>0.543</td>
		<td>0.482</td>
		<td>0.536</td>
		<td>0.544</td>
		<td>0.544</td>
		<td>0.526</td>
		<td>0.541</td>
		<td>0.096</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 3</td>
		<td>0.528</td>	
		<td>0.514</td>
		<td>0.484</td>
		<td>0.535</td>
		<td>0.55</td>
		<td>0.519</td>
		<td>0.507</td>
		<td>0.561</td>
		<td>0.093</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 4</td>
		<td>0.52</td>
		<td>0.516</td>
		<td>0.462</td>
		<td>0.496</td>
		<td>0.53</td>
		<td>0.523</td>
		<td>0.488</td>
		<td>0.524</td>
		<td>0.097</td>
	</tr>
</table>


## Fluorescence per OD

### uM Fluorescein / OD

<table style="width:100%" id="parts">
	<tr>
		<th>Hour 0:</th>
		<th>Neg. Control</th>
		<th>Pos. Control</th>
		<th>Device 1</th>
		<th>Device 2</th>
		<th>Device 3</th>
		<th>Device 4</th>
		<th>Device 5</th>
		<th>Device 6</th>
	</tr>
	<tr>
		<td>Colony 1, Replicate 1</td>	
		<td>0.066</td>
		<td>0.382</td>
		<td>0.154</td>
		<td>0.126</td>
		<td>0.015</td>
		<td>0.118</td>
		<td>0.108</td>
		<td>0.145</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 2</td>
		<td>0.137</td>
		<td>0.283</td>
		<td>0.135</td>
		<td>0.279</td>
		<td>-0.042</td>
		<td>0.062</td>
		<td>0.068</td>
		<td>0.096</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 3</td>
		<td>0.130</td>
		<td>0.302</td>
		<td>0.131</td>
		<td>0.233</td>
		<td>-0.027</td>
		<td>0.072</td>
		<td>0.069</td>
		<td>0.101</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 4</td>	
		<td>0.122</td>	
		<td>0.293</td>
		<td>0.153</td>
		<td>0.214</td>
		<td>-0.018</td>
		<td>0.091</td>
		<td>0.063</td>
		<td>0.106</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 1</td>
		<td>0.100</td>
		<td>0.170</td>
		<td>0.115</td>
		<td>0.231</td>
		<td>-0.032</td>
		<td>0.057</td>
		<td>0.169</td>
		<td>0.059</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 2</td>
		<td>0.193</td>
		<td>0.270</td>
		<td>0.167</td>
		<td>0.278</td>
		<td>-0.011</td>
		<td>0.083</td>
		<td>0.196</td>
		<td>0.114</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 3</td>
		<td>0.146</td>
		<td>0.229</td>
		<td>0.109</td>
		<td>0.221</td>
		<td>-0.036</td>
		<td>0.071</td>
		<td>0.127</td>
		<td>0.088</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 4</td>
		<td>0.037</td>	
		<td>0.209</td>	
		<td>0.139</td>
		<td>0.056</td>	
		<td>-0.064</td>
		<td>0.046</td>
		<td>0.213</td>
		<td>0.050</td>
	</tr>
</table>   

<table style="width:100%" id="parts">
	<tr>
		<th>Hour 6:</th>
		<th>Neg. Control</th>
		<th>Pos. Control</th>
		<th>Device 1</th>
		<th>Device 2</th>
		<th>Device 3</th>
		<th>Device 4</th>
		<th>Device 5</th>
		<th>Device 6</th>
	</tr>
	<tr>
		<td>Colony 1, Replicate 1</td>	
		<td>0.003</td>	
		<td>0.178</td>	
		<td>0.086</td>	
		<td>0.139</td>	
		<td>0.003</td>	
		<td>0.006</td>	
		<td>0.014</td>	
		<td>0.066</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 2</td>
		<td>0.005</td>	
		<td>0.167</td>	
		<td>0.083</td>	
		<td>0.145</td>	
		<td>0.005</td>	
		<td>0.009</td>	
		<td>0.014</td>	
		<td>0.069</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 3</td>
		<td>0.005</td>	
		<td>0.156</td>	
		<td>0.079</td>	
		<td>0.144</td>	
		<td>0.006</td>	
		<td>0.008</td>	
		<td>0.015</td>	
		<td>0.072</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 4</td>	
		<td>0.006</td>	
		<td>0.160</td>	
		<td>0.079</td>	
		<td>0.145</td>	
		<td>0.007</td>	
		<td>0.009</td>	
		<td>0.018</td>	
		<td>0.068</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 1</td>
		<td>0.005</td>	
		<td>0.136</td>	
		<td>0.095</td>	
		<td>0.137</td>	
		<td>0.005</td>	
		<td>0.010</td>	
		<td>0.034</td>	
		<td>0.062</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 2</td>
		<td>0.006</td>	
		<td>0.139</td>	
		<td>0.105</td>	
		<td>0.143</td>	
		<td>0.006</td>	
		<td>0.011</td>	
		<td>0.039</td>	
		<td>0.065</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 3</td>
		<td>0.007</td>	
		<td>0.145</td>	
		<td>0.103</td>	
		<td>0.143</td>	
		<td>0.005</td>	
		<td>0.013</td>	
		<td>0.040</td>	
		<td>0.059</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 4</td>
		<td>0.003</td>	
		<td>0.162</td>	
		<td>0.123</td>	
		<td>0.152</td>	
		<td>0.005</td>	
		<td>0.011</td>	
		<td>0.049</td>	
		<td>0.069</td>
	</tr>
</table>



### Net Fluorescein a.u.

<table style="width:100%" id="parts">
	<tr>
		<th>Hour 0:</th>
		<th>Neg. Control</th>
		<th>Pos. Control</th>
		<th>Device 1</th>
		<th>Device 2</th>
		<th>Device 3</th>
		<th>Device 4</th>
		<th>Device 5</th>
		<th>Device 6</th>
	</tr>
	<tr>
		<td>Colony 1, Replicate 1</td>
		<td>11.64</td>	
		<td>59.32</td>	
		<td>23.96</td>	
		<td>24.82</td>	
		<td>2.83</td>
		<td>19.53</td>	
		<td>14.52</td>	
		<td>22.51</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 2</td>
		<td>22.70</td>	
		<td>43.92</td>	
		<td>15.41</td>	
		<td>31.76</td>	
		<td>-5.27</td>	
		<td>7.72</td>
		<td>6.30</td>
		<td>14.89</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 3</td>
		<td>28.24</td>
		<td>53.23</td>
		<td>17.59</td>
		<td>36.23</td>
		<td>-4.43</td>
		<td>11.21</td>
		<td>9.35</td>
		<td>18.81</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 4</td>
		<td>23.96</td>	
		<td>45.56</td>	
		<td>17.48</td>	
		<td>28.79</td>	
		<td>-2.66</td>	
		<td>11.28</td>	
		<td>6.56</td>
		<td>15.36</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 1</td>
		<td>21.81</td>	
		<td>35.19</td>	
		<td>16.70</td>	
		<td>69.53</td>	
		<td>-5.65</td>	
		<td>10.71</td>	
		<td>52.54</td>	
		<td>12.16</td>	
	</tr>
	<tr>
		<td>Colony 2, Replicate 2</td>
		<td>38.05</td>	
		<td>44.72</td>	
		<td>19.04</td>	
		<td>34.55</td>	
		<td>-1.52</td>	
		<td>11.23</td>	
		<td>28.45</td>	
		<td>17.78</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 3</td>
		<td>31.80</td>	
		<td>42.74</td>	
		<td>15.87</td>	
		<td>32.03</td>	
		<td>-6.41</td>	
		<td>13.28</td>	
		<td>30.29</td>	
		<td>16.36</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 4</td>
		<td>7.73</td>
		<td>38.93</td>	
		<td>20.15</td>	
		<td>8.66</td>
		<td>-9.92</td>	
		<td>6.60</td>
		<td>33.04</td>	
		<td>7.84</td>
	</tr>
</table>

<table style="width:100%" id="parts">
	<tr>
		<th>Hour 6:</th>
		<th>Neg. Control</th>
		<th>Pos. Control</th>
		<th>Device 1</th>
		<th>Device 2</th>
		<th>Device 3</th>
		<th>Device 4</th>
		<th>Device 5</th>
		<th>Device 6</th>
	</tr>
	<tr>
		<td>Colony 1, Replicate 1</td>
		<td>17.37</td>
		<td>793.26</td>	
		<td>351.36</td>	
		<td>638.58</td>	
		<td>15.63</td>
		<td>28.08</td>
		<td>66.53</td>
		<td>342.12</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 2</td>
		<td>26.70</td>
		<td>759.50</td>	
		<td>334.93</td>	
		<td>662.65</td>	
		<td>24.67</td>
		<td>41.61</td>
		<td>65.90</td>
		<td>333.82</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 3</td>
		<td>27.84</td>
		<td>715.38</td>	
		<td>329.02</td>	
		<td>643.51</td>	
		<td>29.00</td>
		<td>41.07</td>
		<td>70.77</td>
		<td>327.73</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 4</td>
		<td>29.82</td>
		<td>724.00</td>
		<td>323.41</td>
		<td>600.97</td>	
		<td>35.22</td>
		<td>44.14</td>
		<td>77.51</td>
		<td>334.09</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 1</td>
		<td>24.12</td>
		<td>631.60</td>	
		<td>400.91</td>	
		<td>616.52</td>	
		<td>25.66</td>
		<td>47.64</td>
		<td>157.92</td>	
		<td>299.65</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 2</td>
		<td>27.73</td>
		<td>644.75</td>	
		<td>420.79</td>	
		<td>649.82</td>	
		<td>27.92</td>
		<td>51.88</td>
		<td>172.24</td>	
		<td>297.58</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 3</td>
		<td>30.80</td>
		<td>633.32</td>	
		<td>415.51</td>	
		<td>656.88</td>	
		<td>25.22</td>
		<td>58.43</td>
		<td>173.59</td>	
		<td>286.40</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 4</td>
		<td>13.89</td>
		<td>702.26</td>	
		<td>463.15</td>	
		<td>627.04</td>	
		<td>22.83</td>
		<td>48.48</td>
		<td>196.84</td>	
		<td>306.56</td>
	</tr>
</table>

### Net Abs 600 

<table style="width:100%" id="parts">
	<tr>
		<th>Hour 0:</th>
		<th>Neg. Control</th>
		<th>Pos. Control</th>
		<th>Device 1</th>
		<th>Device 2</th>
		<th>Device 3</th>
		<th>Device 4</th>
		<th>Device 5</th>
		<th>Device 6</th>
	</tr>
	<tr>
		<td>Colony 1, Replicate 1</td>
		<td>0.017</td>	
		<td>0.015</td>	
		<td>0.015</td>	
		<td>0.019</td>	
		<td>0.018</td>	
		<td>0.016</td>	
		<td>0.013</td>	
		<td>0.015</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 2</td>
		<td>0.016</td>	
		<td>0.015</td>	
		<td>0.011</td>	
		<td>0.011</td>	
		<td>0.012</td>	
		<td>0.012</td>	
		<td>0.009</td>	
		<td>0.015</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 3</td>
		<td>0.021</td>	
		<td>0.017</td>	
		<td>0.013</td>	
		<td>0.015</td>	
		<td>0.016</td>	
		<td>0.015</td>	
		<td>0.013</td>	
		<td>0.018</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 4</td>
		<td>0.019</td>	
		<td>0.015</td>	
		<td>0.011</td>	
		<td>0.013</td>	
		<td>0.014</td>	
		<td>0.012</td>	
		<td>0.010</td>	
		<td>0.014</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 1</td>
		<td>0.021</td>	
		<td>0.020</td>	
		<td>0.014</td>	
		<td>0.029</td>	
		<td>0.017</td>	
		<td>0.018</td>	
		<td>0.030</td>	
		<td>0.020</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 2</td>
		<td>0.019</td>	
		<td>0.016</td>	
		<td>0.011</td>	
		<td>0.012</td>	
		<td>0.013</td>	
		<td>0.013</td>	
		<td>0.014</td>	
		<td>0.015</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 3</td>
		<td>0.021</td>	
		<td>0.018</td>	
		<td>0.014</td>	
		<td>0.014</td>	
		<td>0.017</td>	
		<td>0.018</td>	
		<td>0.023</td>	
		<td>0.018</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 4</td>
		<td>0.020</td>	
		<td>0.018</td>	
		<td>0.014</td>	
		<td>0.015</td>	
		<td>0.015</td>	
		<td>0.014</td>	
		<td>0.015</td>	
		<td>0.015</td>
	</tr>
</table>

<table style="width:100%" id="parts">
	<tr>
		<th>Hour 6:</th>
		<th>Neg. Control</th>
		<th>Pos. Control</th>
		<th>Device 1</th>
		<th>Device 2</th>
		<th>Device 3</th>
		<th>Device 4</th>
		<th>Device 5</th>
		<th>Device 6</th>
	</tr>
	<tr>
		<td>Colony 1, Replicate 1</td>
		<td>0.489</td>	
		<td>0.430</td>	
		<td>0.394</td>	
		<td>0.442</td>	
		<td>0.498</td>	
		<td>0.449</td>	
		<td>0.462</td>	
		<td>0.499</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 2</td>
		<td>0.470</td>	
		<td>0.440</td>	
		<td>0.390</td>	
		<td>0.440</td>	
		<td>0.489</td>	
		<td>0.451</td>	
		<td>0.448</td>	
		<td>0.467</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 3</td>
		<td>0.501</td>	
		<td>0.444</td>	
		<td>0.404</td>	
		<td>0.431</td>	
		<td>0.491</td>	
		<td>0.472</td>	
		<td>0.446</td>	
		<td>0.437</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 4</td>
		<td>0.492</td>	
		<td>0.438</td>	
		<td>0.396</td>	
		<td>0.400</td>	
		<td>0.487</td>	
		<td>0.477</td>	
		<td>0.406</td>	
		<td>0.475</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 1</td>
		<td>0.454</td>	
		<td>0.449</td>	
		<td>0.408</td>	
		<td>0.433</td>	
		<td>0.459</td>	
		<td>0.471</td>	
		<td>0.450</td>	
		<td>0.470</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 2</td>
		<td>0.470</td>	
		<td>0.447</td>	
		<td>0.386</td>	
		<td>0.440</td>	
		<td>0.448</td>	
		<td>0.448</td>	
		<td>0.430</td>	
		<td>0.445</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 3</td>
		<td>0.435</td>	
		<td>0.421</td>	
		<td>0.391</td>	
		<td>0.442</td>	
		<td>0.457</td>	
		<td>0.426</td>	
		<td>0.414</td>	
		<td>0.468</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 4</td>
		<td>0.423</td>	
		<td>0.419</td>	
		<td>0.365</td>	
		<td>0.399</td>	
		<td>0.433</td>	
		<td>0.426</td>	
		<td>0.391</td>	
		<td>0.427</td>
	</tr>
</table>

## Fluorescence per Particle

### MEFL / particle

<table style="width:100%" id="parts">
	<tr>
		<th>Hour 0:</th>
		<th>Neg. Control</th>
		<th>Pos. Control</th>
		<th>Device 1</th>
		<th>Device 2</th>
		<th>Device 3</th>
		<th>Device 4</th>
		<th>Device 5</th>
		<th>Device 6</th>
	</tr>
	<tr>
		<td>Colony 1, Replicate 1</td>
		<td>2.24E+03</td>	
		<td>1.29E+04</td>	
		<td>5.23E+03</td>	
		<td>4.27E+03</td>	
		<td>5.14E+02</td>	
		<td>3.99E+03</td>	
		<td>3.65E+03</td>	
		<td>4.91E+03</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 2</td>
		<td>4.64E+03</td>	
		<td>9.58E+03</td>	
		<td>4.58E+03</td>	
		<td>9.45E+03</td>	
		<td>-1.44E+03</td>	
		<td>2.11E+03</td>	
		<td>2.29E+03</td>	
		<td>3.25E+03</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 3</td>
		<td>4.40E+03</td>
		<td>1.02E+04</td>
		<td>4.43E+03</td>
		<td>7.90E+03</td>
		<td>-9.06E+02</td>	
		<td>2.44E+03</td>
		<td>2.35E+03</td>
		<td>3.42E+03</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 4</td>
		<td>4.12E+03</td>
		<td>9.94E+03</td>
		<td>5.20E+03</td>
		<td>7.24E+03</td>
		<td>-6.22E+02</td>	
		<td>3.07E+03</td>	
		<td>2.15E+03</td>
		<td>3.59E+03</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 1</td>
		<td>3.40E+03</td>
		<td>5.76E+03</td>
		<td>3.90E+03</td>
		<td>7.84E+03</td>
		<td>-1.09E+03</td>	
		<td>1.95E+03</td>
		<td>5.73E+03</td>
		<td>1.99E+03</td>
	</tr>
		<tr>
		<td>Colony 2, Replicate 2</td>
		<td>6.55E+03</td>	
		<td>9.14E+03</td>	
		<td>5.66E+03</td>	
		<td>9.42E+03</td>	
		<td>-3.82E+02</td>	
		<td>2.83E+03</td>	
		<td>6.65E+03</td>	
		<td>3.88E+03</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 3</td>
		<td>4.95E+03</td>	
		<td>7.77E+03</td>	
		<td>3.71E+03</td>	
		<td>7.48E+03</td>	
		<td>-1.23E+03</td>	
		<td>2.41E+03</td>	
		<td>4.31E+03</td>	
		<td>2.97E+03</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 4</td>
		<td>1.26E+03</td>
		<td>7.08E+03</td>
		<td>4.71E+03</td>
		<td>1.89E+03</td>
		<td>-2.16E+03</td>	
		<td>1.54E+03</td>
		<td>7.21E+03</td>
		<td>1.71E+03</td>
	</tr>
</table>

<table style="width:100%" id="parts">
	<tr>
		<th>Hour 6:</th>
		<th>Neg. Control</th>
		<th>Pos. Control</th>
		<th>Device 1</th>
		<th>Device 2</th>
		<th>Device 3</th>
		<th>Device 4</th>
		<th>Device 5</th>
		<th>Device 6</th>
	</tr>
	<tr>
		<td>Colony 1, Replicate 1</td>
		<td>1.16E+02</td>	
		<td>6.03E+03</td>	
		<td>2.92E+03</td>	
		<td>4.73E+03</td>	
		<td>1.03E+02</td>	
		<td>2.05E+02</td>	
		<td>4.71E+02</td>	
		<td>2.24E+03</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 2</td>
		<td>1.86E+02</td>	
		<td>5.65E+03</td>	
		<td>2.81E+03</td>	
		<td>4.93E+03</td>	
		<td>1.65E+02</td>	
		<td>3.02E+02</td>	
		<td>4.81E+02</td>	
		<td>2.34E+03</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 3</td>
		<td>1.82E+02</td>	
		<td>5.27E+03</td>	
		<td>2.66E+03</td>	
		<td>4.88E+03</td>	
		<td>1.93E+02</td>	
		<td>2.85E+02</td>	
		<td>5.19E+02</td>	
		<td>2.45E+03</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 4</td>
		<td>1.98E+02</td>	
		<td>5.41E+03</td>	
		<td>2.67E+03</td>	
		<td>4.91E+03</td>	
		<td>2.37E+02</td>	
		<td>3.03E+02</td>	
		<td>6.25E+02</td>	
		<td>2.30E+03</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 1</td>
		<td>1.74E+02</td>	
		<td>4.60E+03</td>	
		<td>3.21E+03</td>	
		<td>4.66E+03</td>	
		<td>1.83E+02</td>	
		<td>3.31E+02</td>	
		<td>1.15E+03</td>	
		<td>2.09E+03</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 2</td>
		<td>1.93E+02</td>	
		<td>4.72E+03</td>	
		<td>3.57E+03</td>	
		<td>4.83E+03</td>	
		<td>2.04E+02</td>	
		<td>3.79E+02</td>	
		<td>1.31E+03</td>	
		<td>2.19E+03</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 3</td>
		<td>2.32E+02</td>	
		<td>4.92E+03</td>	
		<td>3.48E+03</td>	
		<td>4.86E+03</td>	
		<td>1.81E+02</td>	
		<td>4.49E+02</td>	
		<td>1.37E+03</td>	
		<td>2.00E+03</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 4</td>
		<td>1.07E+02</td>	
		<td>5.48E+03</td>	
		<td>4.15E+03</td>	
		<td>5.14E+03</td>	
		<td>1.72E+02</td>	
		<td>3.72E+02</td>	
		<td>1.65E+03</td>	
		<td>2.35E+03</td>
	</tr>
</table>

### Net Fluorescein a.u.

<table style="width:100%" id="parts">
	<tr>
		<th>Hour 0:</th>
		<th>Neg. Control</th>
		<th>Pos. Control</th>
		<th>Device 1</th>
		<th>Device 2</th>
		<th>Device 3</th>
		<th>Device 4</th>
		<th>Device 5</th>
		<th>Device 6</th>
	</tr>
	<tr>
		<td>Colony 1, Replicate 1</td>
		<td>11.64</td>	
		<td>59.32</td>	
		<td>23.96</td>	
		<td>24.82</td>	
		<td>2.83</td>	
		<td>19.53</td>	
		<td>14.52</td>	
		<td>22.51</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 2</td>
		<td>22.70</td>	
		<td>43.92</td>	
		<td>15.41</td>	
		<td>31.76</td>	
		<td>-5.27</td>	
		<td>7.72</td>
		<td>6.30</td>
		<td>14.89</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 3</td>
		<td>28.24</td>	
		<td>53.23</td>	
		<td>17.59</td>	
		<td>36.23</td>	
		<td>-4.43</td>	
		<td>11.21</td>	
		<td>9.35</td>
		<td>18.81</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 4</td>
		<td>23.96</td>	
		<td>45.56</td>	
		<td>17.48</td>	
		<td>28.79</td>	
		<td>-2.66</td>	
		<td>11.28</td>	
		<td>6.56</td>
		<td>15.36</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 1</td>
		<td>21.81</td>	
		<td>35.19</td>	
		<td>16.70</td>	
		<td>69.53</td>	
		<td>-5.65</td>	
		<td>10.71</td>	
		<td>52.54</td>	
		<td>12.16</td>
	</tr>
		<tr>
		<td>Colony 2, Replicate 2</td>
		<td>38.05</td>	
		<td>44.72</td>	
		<td>19.04</td>	
		<td>34.55</td>	
		<td>-1.52</td>	
		<td>11.23</td>	
		<td>28.45</td>	
		<td>17.78</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 3</td>
		<td>31.80</td>	
		<td>42.74</td>	
		<td>15.87</td>	
		<td>32.03</td>	
		<td>-6.41</td>	
		<td>13.28</td>	
		<td>30.29</td>	
		<td>16.36</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 4</td>
		<td>7.73</td>	
		<td>38.93</td>	
		<td>20.15</td>	
		<td>8.66</td>	
		<td>-9.92</td>
		<td>6.60</td>
		<td>33.04</td>
		<td>7.84</td>
	</tr>
</table>

<table style="width:100%" id="parts">
	<tr>
		<th>Hour 6:</th>
		<th>Neg. Control</th>
		<th>Pos. Control</th>
		<th>Device 1</th>
		<th>Device 2</th>
		<th>Device 3</th>
		<th>Device 4</th>
		<th>Device 5</th>
		<th>Device 6</th>
	</tr>
	<tr>
		<td>Colony 1, Replicate 1</td>
		<td>17.37</td>
		<td>793.26</td>	
		<td>351.36</td>	
		<td>638.58</td>	
		<td>15.63</td>
		<td>28.08</td>
		<td>66.53</td>
		<td>342.12</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 2</td>
		<td>26.70</td>
		<td>759.50</td>	
		<td>334.93</td>	
		<td>662.65</td>	
		<td>24.67</td>
		<td>41.61</td>
		<td>65.90</td>
		<td>333.82</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 3</td>
		<td>27.84</td>
		<td>715.38</td>	
		<td>329.02</td>	
		<td>643.51</td>	
		<td>29.00</td>
		<td>41.07</td>
		<td>70.77</td>
		<td>327.73</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 4</td>
		<td>29.82</td>
		<td>724.00</td>	
		<td>323.41</td>	
		<td>600.97</td>	
		<td>35.22</td>
		<td>44.14</td>
		<td>77.51</td>
		<td>334.09</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 1</td>
		<td>24.12</td>
		<td>631.60</td>	
		<td>400.91</td>	
		<td>616.52</td>	
		<td>25.66</td>
		<td>47.64</td>
		<td>157.92</td>
		<td>299.65</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 2</td>
		<td>27.73</td>
		<td>644.75</td>	
		<td>420.79</td>	
		<td>649.82</td>	
		<td>27.92</td>
		<td>51.88</td>
		<td>172.24</td>	
		<td>297.58</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 3</td>
		<td>30.80</td>
		<td>633.32</td>	
		<td>415.51</td>	
		<td>656.88</td>	
		<td>25.22</td>
		<td>58.43</td>
		<td>173.59</td>	
		<td>286.40</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 4</td>
		<td>13.89</td>
		<td>702.26</td>	
		<td>463.15</td>	
		<td>627.04</td>	
		<td>22.83</td>
		<td>48.48</td>
		<td>196.84</td>	
		<td>306.56</td>
	</tr>
</table>

### Net Abs 600

<table style="width:100%" id="parts">
	<tr>
		<th>Hour 0:</th>
		<th>Neg. Control</th>
		<th>Pos. Control</th>
		<th>Device 1</th>
		<th>Device 2</th>
		<th>Device 3</th>
		<th>Device 4</th>
		<th>Device 5</th>
		<th>Device 6</th>
	</tr>
	<tr>
		<td>Colony 1, Replicate 1</td>
		<td>0.017</td>	
		<td>0.015</td>	
		<td>0.015</td>	
		<td>0.019</td>	
		<td>0.018</td>	
		<td>0.016</td>	
		<td>0.013</td>	
		<td>0.015</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 2</td>
		<td>0.016</td>	
		<td>0.015</td>	
		<td>0.011</td>	
		<td>0.011</td>	
		<td>0.012</td>	
		<td>0.012</td>	
		<td>0.009</td>	
		<td>0.015</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 3</td>
		<td>0.021</td>	
		<td>0.017</td>	
		<td>0.013</td>	
		<td>0.015</td>	
		<td>0.016</td>	
		<td>0.015</td>	
		<td>0.013</td>	
		<td>0.018</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 4</td>
		<td>0.019</td>	
		<td>0.015</td>	
		<td>0.011</td>	
		<td>0.013</td>	
		<td>0.014</td>	
		<td>0.012</td>	
		<td>0.010</td>	
		<td>0.014</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 1</td>
		<td>0.021</td>	
		<td>0.020</td>	
		<td>0.014</td>	
		<td>0.029</td>	
		<td>0.017</td>	
		<td>0.018</td>	
		<td>0.030</td>	
		<td>0.020</td>
	</tr>
		<tr>
		<td>Colony 2, Replicate 2</td>
		<td>0.019</td>	
		<td>0.016</td>	
		<td>0.011</td>	
		<td>0.012</td>	
		<td>0.013</td>	
		<td>0.013</td>	
		<td>0.014</td>	
		<td>0.015</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 3</td>
		<td>0.021</td>	
		<td>0.018</td>	
		<td>0.014</td>	
		<td>0.014</td>	
		<td>0.017</td>	
		<td>0.018</td>	
		<td>0.023</td>	
		<td>0.018</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 4</td>
		<td>0.020</td>	
		<td>0.018</td>	
		<td>0.014</td>	
		<td>0.015</td>	
		<td>0.015</td>	
		<td>0.014</td>	
		<td>0.015</td>	
		<td>0.015</td>
	</tr>
</table>

<table style="width:100%" id="parts">
	<tr>
		<th>Hour 6:</th>
		<th>Neg. Control</th>
		<th>Pos. Control</th>
		<th>Device 1</th>
		<th>Device 2</th>
		<th>Device 3</th>
		<th>Device 4</th>
		<th>Device 5</th>
		<th>Device 6</th>
	</tr>
	<tr>
		<td>Colony 1, Replicate 1</td>
		<td>0.489</td>	
		<td>0.430</td>	
		<td>0.394</td>	
		<td>0.442</td>	
		<td>0.498</td>	
		<td>0.449</td>	
		<td>0.462</td>	
		<td>0.499</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 2</td>
		<td>0.470</td>	
		<td>0.440</td>	
		<td>0.390</td>	
		<td>0.440</td>	
		<td>0.489</td>	
		<td>0.451</td>	
		<td>0.448</td>	
		<td>0.467</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 3</td>
		<td>0.501</td>	
		<td>0.444</td>	
		<td>0.404</td>	
		<td>0.431</td>	
		<td>0.491</td>	
		<td>0.472</td>	
		<td>0.446</td>	
		<td>0.437</td>
	</tr>
	<tr>
		<td>Colony 1, Replicate 4</td>
		<td>0.492</td>	
		<td>0.438</td>	
		<td>0.396</td>	
		<td>0.400</td>	
		<td>0.487</td>	
		<td>0.477</td>	
		<td>0.406</td>	
		<td>0.475</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 1</td>
		<td>0.454</td>	
		<td>0.449</td>	
		<td>0.408</td>	
		<td>0.433</td>	
		<td>0.459</td>	
		<td>0.471</td>	
		<td>0.450</td>	
		<td>0.470</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 2</td>
		<td>0.470</td>	
		<td>0.447</td>	
		<td>0.386</td>	
		<td>0.440</td>	
		<td>0.448</td>	
		<td>0.448</td>	
		<td>0.430</td>	
		<td>0.445</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 3</td>
		<td>0.435</td>	
		<td>0.421</td>	
		<td>0.391</td>	
		<td>0.442</td>	
		<td>0.457</td>	
		<td>0.426</td>	
		<td>0.414</td>	
		<td>0.468</td>
	</tr>
	<tr>
		<td>Colony 2, Replicate 4</td>
		<td>0.423</td>	
		<td>0.419</td>	
		<td>0.365</td>	
		<td>0.399</td>	
		<td>0.433</td>	
		<td>0.426</td>	
		<td>0.391</td>	
		<td>0.427</td>
	</tr>
</table>
